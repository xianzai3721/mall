        ...
        <div class="navbar">
          ...
          <div class="navbar-right-container" style="display: flex;">
            <div class="navbar-menu-container">
              ...
              <a href="javascript:void(0)" v-show="!$store.state.isLogin" class="navbar-link">Login</a>
              <a href="javascript:void(0)" v-show="$store.state.isLogin" class="navbar-link">{{$store.state.nickName}}</a>
              <a href="javascript:void(0)" v-show="$store.state.isLogin" class="navbar-link">Logout</a>
              <div class="navbar-cart-container">
                <span class="navbar-cart-count">{{count}}</span>
                  ...
                  <ul>
                    <li v-for="item in goodslist">
                      ...
                      <div class="main">
                        ...
                        <div class="btn-area">
                          <a href="javascript:;" class="btn btn--m" @click="add()">加入购物车</a>
                        </div>
                      </div>
                    </li>
                  </ul>
...
<script>
    ...
    export default{
      ...
      methods:{
        getGoodsList(flag){
          this.loading = true;
          this.$axios.get('http://localhost:8080/api/goods',{})
            .then((data)=>{
              if(data.data.errno == 0){
                if(flag){
                  this.goodslist = this.goodslist.concat(data.data.data.result);

                  if(data.data.data.result.count==0){
                    this.busy = true;
                  }else{
                    this.busy = false;
                  }
                }else{
                  this.goodslist = data.data.data.result;
                  this.busy = false;
                }
              }else{
                this.goodslist = [];
        ...
        loadMore() {
          ...
          setTimeout(() => {
            this.page++;
            this.getGoodsList(true);//这里请求接口去拿数据，实际应该是调用一个请求数据的方法
          }, 1000);
        },
        add(){
          this.$store.dispatch('updateCartCountAction',1)
        }
      },
        created(){
